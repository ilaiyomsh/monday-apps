...
**Phase A — Full gate (1 agent).** `pnpm install --frozen-lockfile` (a clean install is the
only place a dependency batch that broke resolution shows up), then wiring audit,
eager-import audit, typecheck, lint, build, full test suites for both workspaces, and the
error-kit drift suite. Also measures the SPA bundle with `du -sk apps/twyst-your-status/dist`
the same way the baseline did. **Any failure forces the verdict to `ISSUES_FOUND`,**
...
Fresh context, base SHA only. Walks every commit hunting behaviour changes, deleted-but-
referenced code (including string routes and paths named in `scripts/error-wiring-audit.mjs`
...
OAuth scopes, routes), and any file outside the app.
