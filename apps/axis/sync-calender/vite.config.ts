...
export default defineConfig({
  ...
  build: {
    ...
    // deletes public/admin/**/*.map before code:push, so client source is never
    // served (this also fixes the prior sourcemap:true public leak). Maps are kept
    // as a CI artifact for stack symbolication if client telemetry is enabled.
    // Server code runs from source (already readable). See docs/LOGGING-ARCHITECTURE.md §6.
    sourcemap: 'hidden',
  },
  server: {
    port: 5173,
