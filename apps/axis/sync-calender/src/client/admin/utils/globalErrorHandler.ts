/**
 * globalErrorHandler.ts — global error capture funnelled into the app logger.
 *
 * VENDORED from packages/error-kit/src/browser/globalErrorHandler.ts. Only the Logger type
 * import path is adjusted (this app vendors the error-kit stack — see axiomBrowserTransport.ts).
 *
 * A CAPTURE-PHASE window 'error' listener catches resource-load failures (script/link/img —
 * these do NOT bubble, so only capture sees them); an idempotency guard (a window flag with a
 * module-level fallback for non-browser/test); an optional chunk-error seam. Resource failures
 * log at WARN (a missing asset should not pop an error toast but must be visible with url+tag);
 * uncaught errors and unhandled rejections log at ERROR through the single log-once funnel.
 * Import-order-safe: no top-level side effects, safe to call before render.
 */
