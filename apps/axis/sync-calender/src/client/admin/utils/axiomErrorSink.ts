/**
 * axiomErrorSink.ts — bridges this app's `logger` records into the hardened Axiom browser
 * transport (direct ingest into the SHARED `app-errors` dataset, discriminated by `app`).
 *
 * VENDORED from packages/error-kit/src/browser/axiomSink.ts (the canonical source, WITH its
 * 5 hardening fixes). Adjustments (this app is a server/monday-code app whose client bundle
 * cannot resolve workspace deps, so the stack is vendored):
 *   - imports point at the vendored transport (./axiomBrowserTransport) and this app's
 *     logger types (../lib/logger);
 *   - the domain discriminator is read from `record.domainKind` (this repo's logger + server
 *     sink wire — src/services/axiomServerSink.js — use `domainKind`; the emitted Axiom event
 *     field is `kind` either way);
 *   - the activation gate additionally requires `options.app` (task: PROD && VITE_AXIOM_DATASET
 *     && VITE_AXIOM_TOKEN && VITE_AXIOM_APP).
 *
 * Activation gate: ships ONLY when `active` resolves true — by default `import.meta.env.PROD`
 * AND dataset/token/app all present (the consumer reads VITE_AXIOM_* and passes them in). Dev
 * server, tunnel, and tests are structurally inert: the transport is an inert handle and
 * attachAxiomSink() degrades to a no-op.
 *
 * PRIVACY (defense in depth): the sink NEVER copies record.data or context query/response.
 * error.message ships ONLY as `err_msg` via scrubMessage (emails / tokens&hex>=16 /
 * digit-runs>=7 redacted, capped 200). The transport's exact-key allowlist backstops it.
 */
import {
...
} from './axiomBrowserTransport';
import type { Logger, LogRecord } from '../lib/logger';

// Rank table — DEBUG < INFO < WARN < ERROR.
const RANK: Record<string, number> = { DEBUG: 0, INFO: 1, WARN: 2, ERROR: 3 };
const REMOTE_LEVEL_KEY = 'sync-calender-admin:remoteLogLevel';

// Guarded prod probe. Vite inlines `import.meta.env.PROD`; read under a typeof guard so the
// reference never throws in a non-Vite environment, and a consumer can override via options.active.
...
export interface AxiomSinkOptions {
  /** Dataset discriminator — the app slug. Stamped as the `app` field. */
  app: string;
 ...
 * has whitespace before the '@', so it can never be mistaken for a frame and leak
 ...
 * 1000 to bound regex work, final slice 200.
 */
...
/**
 * Maps EXACTLY the allowlisted fields — never record.data or context query/response;
 * error.message only as scrubbed err_msg. The transport stamps `_time` at enqueue and
...
export function mapRecordToEvent(record: LogRecord): AxiomEventInput {
  ...
  const ev: AxiomEventInput = {
    ...
    // domain discriminator: error (default) | usage | health. track()/health() set
    // record.domainKind; NEVER ship the rendering `kind`.
    kind: String(r.domainKind ?? 'error'),
  };
...
export function attachAxiomSink(logger: Logger, options: AxiomSinkOptions): () => void {
  ...
  const active = options.active ?? (isProd() && Boolean(dataset) && Boolean(token) && Boolean(options.app));
  if (!active) return () => {};

  // Check the attach guard BEFORE building the transport. createAxiomBrowserTransport disposes
  // any transport already registered for this app, so a second attach that reached the
  // constructor would tear down the live transport, then hit the guard and no-op — orphaning
  // the new transport. Bailing out here keeps the first, live transport intact.
  const g: { __SYNC_CALENDER_AXIOM_SINK_ATTACHED__?: boolean } =
    typeof globalThis !== 'undefined' ? (globalThis as typeof g) : {};
  if (g.__SYNC_CALENDER_AXIOM_SINK_ATTACHED__) return () => {}; // survives HMR module re-eval
  ...
  g.__SYNC_CALENDER_AXIOM_SINK_ATTACHED__ = true; // set once we own a transport, before replay
  ...
  return () => {
    ...
    g.__SYNC_CALENDER_AXIOM_SINK_ATTACHED__ = false;
    if (activeTransport === transport) activeTransport = null;
  };
}

/**
 * The active transport's live stats, or an inert `{ enabled: false }` when no sink is attached.
 */
...
/**
 * True once attachAxiomSink has attached an active transport (prod build with dataset +
 * token + app present). Gates optional identity-resolution API calls so telemetry never
 * costs an API round-trip when the sink is structurally inert (dev / tunnel / tests).
 */
