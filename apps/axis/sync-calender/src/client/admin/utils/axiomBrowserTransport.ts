 ...
 * key). Only the file/header name differs — this app is a server (monday-code) app whose
 * client bundle cannot resolve workspace deps, so the stack is vendored, not imported.
 *
...
export interface AxiomTransportOptions {
  dataset: string; token: string; app: string;         // 'sync-calender-admin'
...
export function createAxiomBrowserTransport(options: AxiomTransportOptions): AxiomTransport {
  ...
  async function doFlush(reason: FlushReason): Promise<void> {
    ...
    // drain: cutBatchHidden() empties the queue synchronously, so running alongside the
    // drain is safe — the drain loop re-checks queue.length after its await and exits cleanly.
    if (reason === 'hidden') {
   ...
   * enqueue · dedup (fixed window, transport-tag bypass, bounded map) · session cap
   * (exactly-once events_dropped meta, meta bypasses the cap) · queue cap drop-oldest · schedule.
   */
