/**
 * axiomTransport.ts — shared browser log transport for direct Axiom ingest.
 *
 * Normative spec: Axis/TRACKER-AXIOM-EXECUTION-PLAN.md §3 (§3.1 public API,
 * §3.2 internals, §3.3 sanitizer precedence, §3.4 activation gate note).
 *
 * Structural constraints (load-bearing):
 * - NO React imports, NO `import.meta` anywhere — env is injected by the consumer
 *   (the activation gate lives in the consumer's sink, §3.4). Must typecheck under
 *   both app-core `tsc --noEmit` (strict, types: []) and Day-off `tsc -b`.
...
const ERR_MSG_MAX_LEN = 200; // scrubbed error.message (see errors/axiomSink.ts scrubMessage)
