 ...
 *
 * TypeScript port of .claude/skills/error-guard/templates/axiomBrowserTransport.js
 * (itself a faithful port of the Axis app-core transport). Logic is unchanged; only
 * types were added. If a defect is found here, fix the template + app-core too.
 *
 * Structural constraints (load-bearing):
 * - NO React imports, NO `import.meta` anywhere — env is injected by the consumer (the
 *   activation gate lives in axiomErrorSink.ts).
 * - Real globals (fetch/window/document) are referenced ONLY as guarded defaults for the
 *   injectable seams — so the transport is fully testable in plain Node/jsdom.
 * - `enqueue`/`flush` never throw to the caller. Every ship failure emits exactly one
 *   console.error starting '[axiom-transport] '.
 *
 * Event input contract: FLAT objects — strings + finite numbers only (the sanitizer enforces
 * an exact-key allowlist; everything else is dropped, never shipped).
 */
...
/** Flat event envelope (strings + finite numbers). The sanitizer enforces the allowlist. */
...
export interface AxiomBrowserTransport {
...
export interface CreateTransportOptions {
  ...
  fetchFn?: (url: string, init: RequestInit) => Promise<{ ok: boolean; status?: number } | undefined>;
...
function buildAllowlist(caps: TransportCaps): Record<string, number> {
...
const REG = new Map<string, AxiomBrowserTransport>();

export function createAxiomBrowserTransport(options: CreateTransportOptions): AxiomBrowserTransport {
  ...
  const queue: Array<Record<string, string | number>> = [];
  ...
  /** Batch-assembly enrichment: context first, event fields win on collision, statics last. */
  function envelope(ev: Record<string, string | number>): Record<string, string | number> {
  ...
  /** Hidden path: single POST ≤batchMaxBytes carrying the NEWEST events, order preserved. */
  ...
  /** One POST. At-most-once: the batch was already cut from the queue and is never re-queued. */
  ...
  /**
   * Enqueue pipeline (order is contractual): disposed→drop · sanitize + stamp `_time` at
   * enqueue · dedup (fixed window, transport-tag bypass, bounded map) · session cap (exactly-once
   * events_dropped meta, meta bypasses the cap) · queue cap drop-oldest · schedule.
   */
  function enqueueEvent(input: TransportEvent, meta: boolean): void {
    if (disposed) return;
  ...
  const handle: AxiomBrowserTransport = {
    enqueue(event: TransportEvent) {
      try {
    ...
    setContext(ctx: TransportContext) {
