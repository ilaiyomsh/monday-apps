...
export function useReferences(discussionId) {
  ...
  const clearDocuments = useCallback(async () => {
    ...
    try {
      ...
      if (!recreated) {
        logger.error('useReferences', 'מחיקת מסמכי ההתייחסויות נכשלה — לא נוצר עדכון חדש');
        return false;
      }
      updateIdRef.current = String(recreated.id);
      setUpdateId(String(recreated.id));
      await saveReferencesUpdateId(discussionId, recreated.id);
      ...
      if (!err?.__loggedId) logger.error('useReferences', 'מחיקת מסמכי ההתייחסויות נכשלה', err);
      return false;
    }
  }, [discussionId, html]);
