/**
 * Logger — single-choke-point logging pipeline with debug gating and a sink registry.
 *
 * @overview
 * A dependency-free, drop-in logger for browser (client) monday.com apps. Every public
 * method builds a structured record and routes it through emit(record) — the ONE choke-point.
 ...
 *   4. console rendering: ALL console output lives inside emit (via renderToConsole) — never
 *      call console.* from application code.
 *   5. ring buffer: retains the last RING_BUFFER_SIZE records for replay / flush.
 *   6. fan-out to registered sinks (addSink) — each sink dispatched in its own try/catch so a
 ...
 *
 * @note
 * - Do NOT use console.log/error/warn directly in application code — always go through logger.
...
// Current console level — mutable at runtime (see setLevel / enableDebugLogs).
...
// Ring buffer capacity — retains the most recent records for replay / flush.
...
// log-once counter — assigns a unique id to each Error instance that passes through emit.
let loggedIdCounter = 0;

/**
 * beforeSend transform — a single settable choke-point applied inside emit.
 *
 ...
 * filtering (dropping records whose stack contains no first-party frame — host-page noise
...
// ============================================
// Console rendering — ALL console output is centralized here (called from emit only)
// ============================================
 ...
 * Gated by record.consoleEnabled — when false there is no console output (sinks still receive it).
 */
const renderToConsole = (record) => {
  ...
  switch (kind) {
    case 'simple': {
      // debug / info / warn — a plain colored line
 ...
 * @param {Object} [record.context] - diagnostic context (query / rawResponse / queryWarnings / …)
...
const emit = (record) => {
  ...
  // bags (record.context) do not participate in dedup — they are new objects on every call.
  const err =
  ...
  if (err && typeof err === 'object') {
    if (err.__loggedId !== undefined) {
      // Same instance passing through again — a duplicate record.
      record.duplicate = true;
      record.correlationId = record.correlationId || err.correlationId || err.__loggedId;
    } else {
      // First pass — stamp an id (non-enumerable so it never pollutes serialization / JSON).
      ...
      try {
        ...
        // Frozen / non-configurable object — do not block logging (the record still emits).
      }
  ...
  try {
    ...
    // A broken transform must not take down logging — fall back to the untransformed record.
    // eslint-disable-next-line no-console
    console.error('[logger] beforeSend threw and was ignored:', transformError);
    outgoing = record;
  }
  if (outgoing === null || outgoing === undefined) {
    // Suppressed — do not render, buffer, or dispatch.
    return;
  }
...
const logger = {
  ...
  setLevel: (level) => {
    if (typeof level === 'string') {
      // Use ?? not || because LOG_LEVELS.DEBUG === 0 (falsy) — || would misresolve 'DEBUG'.
   ...
   * @returns {function():void} unsubscribe — calls removeSink(fn)
   */
  addSink: (fn) => {
  ...
  /**
   * The single choke-point — exposed for tests and advanced use. All other methods route here.
   */
  emit,
   ...
   *   target wired yet — the infrastructure is ready, the target is deferred).
  ...
  flush: (url) => {
    ...
    try {
      ...
      // Serialization failure (e.g. circular) — clear intent: do not throw.
      return false;
    }
    try {
      ...
      // Unload path — never throw outward.
      return false;
    }
    // Neither sendBeacon nor fetch available — graceful no-op.
    return false;
  },
  ...
  /**
   * Debug log — detailed development information.
   */
   ...
   * @param {string} message - a STABLE event id, not a raw error.message (the remote
   *   transport ships `message` verbatim; only err_msg is scrubbed)
   * @param {Error|Object|string} [error] - the error instance (or payload)
   * @param {Object} [context] - diagnostic context, shipped on `record.context`.
   *   REQUIRED for AppErrorBoundary, which passes `{ componentStack }` here — the
   *   remote Axiom sink reads the component tree from `record.context.componentStack`
   *   and nowhere else, so dropping this parameter silently strips the component
   *   tree from every shipped render crash.
   */
  ...
  /**
   * API-specific log — before the request.
   */
  ...
  /**
   * API-specific log — after the response.
   */
  ...
  /**
   * API-specific log — an error. Always renders to the console and always reaches sinks.
...
// flush on visibilitychange / beforeunload (infrastructure ready; target deferred — flush
