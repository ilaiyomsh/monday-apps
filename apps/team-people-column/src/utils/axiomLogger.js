 ...
 *
 * WHY THIS EXISTS (platform quirk found during the error-kit migration): the
 * package's mapRecordToEvent reads `record.kind` directly as the Axiom domain
 * discriminator ('error' default | 'usage' | 'health') — that is how app-core's
 * canonical logger (apps/axis/services/app-core/src/logger.ts) shapes it.
 *
 ...
 * track()/health(); undefined otherwise, meaning 'error' by convention — see
 * logger.js track()/health() and the pre-migration vendored axiomErrorSink.js
 * mapRecordToEvent, which read `r.domainKind ?? 'error'`).
 *
 * Feeding this app's raw records straight into the package's sink would ship
 * every plain WARN/DEBUG/INFO record with kind='simple' and every usage/health
 * record with kind='simple' too (instead of 'usage'/'health'), silently
 * breaking the Axiom `kind=` filter dashboards rely on. toAxiomLogger() wraps
 * ONLY the getBuffer/addSink view the package reads and remaps kind ->
 * domainKind ?? 'error' on the way out; the real logger and every other
