/**
 * AppErrorBoundary — the render-phase catch layer for the error-guard standard.
 *
 * Built on the `react-error-boundary` package (declare it as a peer dependency:
 * `pnpm add react-error-boundary`). We do NOT hand-roll a class boundary here —
 ...
 * an application bug — a reload usually fixes it (new bundle, dropped network, or
...
const DefaultFallback = ({ error, resetErrorBoundary }) => {
    ...
    if (isChunkLoadError(error)) {
        return (
            <div style={{ padding: '20px', textAlign: 'center', direction: 'inherit' }} role="alert">
                {/* User-facing Hebrew */}
                <h2>טעינת רכיב נכשלה</h2>
                <p>ייתכן שיצאה גרסה חדשה או שאין חיבור לרשת. רעננו את הדף כדי להמשיך.</p>
                <button
                    type="button"
                    onClick={() => {
    ...
    return (
        <div style={{ padding: '20px', textAlign: 'center', direction: 'inherit' }} role="alert">
            {/* User-facing Hebrew */}
            <h2>משהו השתבש</h2>
            <p>אירעה שגיאה בלתי צפויה. נסו שוב, ואם הבעיה חוזרת רעננו את הדף.</p>
...
/**
 * AppErrorBoundary — wrap any subtree (and the root — see the entry templates).
 *
 * @param {object} props
...
/**
 * useAppErrorFunnel — re-export of react-error-boundary's useErrorBoundary.
 *
 ...
 *           await saveToApi();
 *       } catch (err) {
 *           // Re-route into the boundary instead of swallowing — this satisfies
 ...
 * screen — reserve showBoundary for errors that leave the subtree unusable.
 */
