...
// nets + boot guards vendored from the cross-app server template. Every branch is
// exercised with injected flush/exit/setTimeout seams so the exit path is observed,
// never real. (Mirrors deadline-confirm/tests/process-guards.test.js.)
...
describe('safeBootInit', () => {
  it('returns the initializer result on success without touching logger or exit', () => {
    ...
    const result = safeBootInit(() => backend, 'secure storage init', logger, { exit });
  ...
  it('on init throw: ships a boot-failed ERROR, exits 1, and re-throws', async () => {
    ...
    expect(() =>
      safeBootInit(() => { throw boom; }, 'secure storage init', logger, {
    ...
    await tick();
    expect(logger.logError).toHaveBeenCalledWith('server', 'boot failed: secure storage init', { error: boom });
    expect(exit).toHaveBeenCalledWith(1);
  });
});
