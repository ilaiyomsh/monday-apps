/**
 * axiomServerSink.js — ships the guard server's logger records to Axiom (the
 * SHARED `app-errors` dataset — error-guard references/remote-monitoring.md).
 *
 * VENDORED copy of the cross-app server sink (canonical source: error-guard
 * templates/server/axiomServerSink.js; kept behaviorally in sync by
 * packages/error-kit/test/drift.test.ts). monday-code deploys the app root only,
 * so the guard carries its own copy rather than importing a workspace package.
 * Uses @axiomhq/js (the same client verified in production by the other servers).
 * The SDK batches in the background; flushAxiom() drains the buffer on shutdown.
 *
 * Activation gate: token + dataset + app. These are read through the caller
 * (index.js passes them from EnvironmentVariablesManager, NOT process.env —
 * monday-code injects platform env via the SDK secrets file). Without them
 * attachAxiomServerSink() is inert and the console transport keeps working.
 *
 ...
 * capped 200) + first stack frame + allow-listed numeric/short-string context.
 * OAuth tokens live in record context under keys that are NOT in CTX_ALLOW, so they
 * never reach the wire. This is a sink file — exempt from the no-console rule
