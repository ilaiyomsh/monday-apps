// monday OAuth 2.1 HTTP client — the ONE place that owns the oauth_ms endpoint
// URLs and form-param shapes, so the router (code exchange), the token store
// (refresh) and disconnect (revoke) never triplicate them. monday's new flow:
// PKCE S256 is mandatory, access tokens EXPIRE (no expires_in — the expiry is the
// JWT `exp` claim), refresh tokens are SINGLE-USE and rotate on every refresh, and
// the whole grant dies 6 months after the original authorization.
//
// VENDORED from the reference implementation (apps/telemetry-dashboard/src/
...
/**
 * Decode a JWT's `exp` claim to ms-epoch. DECODE, NOT VERIFY — monday issued this
 * JWT and we have no verification key; the value is used ONLY to schedule proactive
 * refreshes, never for authorization decisions.
 *
 * DELIBERATE DIVERGENCE from the template (monday-oauth §5): the optional `logger`
 * param exists so the parse catch is non-silent (error-guard house rule, CLAUDE.md
 * golden rule #5). It logs a MACHINE CODE only — never token material.
 * @param {unknown} token
 * @param {{ debug?: Function }} [logger]
 * @returns {number|null} ms-epoch expiry, or null when undecodable
 */
export function decodeJwtExpMs(token, logger) {
  ...
  try {
    ...
    // Opaque / non-JWT token — callers fall back to FALLBACK_TTL_MS. Machine code
    // only (no token material) so the catch is not silent.
    if (logger) logger.debug('oauth_jwt_exp_undecodable', 'oauth', { reason: err?.name ?? 'parse_error' });
    return null;
  }
}
 ...
 * }}
 */
export function createMondayOauthClient({ clientId, clientSecret, fetchImpl, logger }) {
  ...
  function toTokens(body) {
    ...
    const expMs = decodeJwtExpMs(accessToken, logger);
    return {
      accessToken,
  ...
  return {
    ...
    /**
     * refresh_token grant. Refresh tokens are SINGLE-USE: a success rotates the
     * pair (persist the new refresh token, discard the old). invalid_grant / 400 /
     * 401 means the refresh token is permanently dead (rotated-away, revoked, or
     * past the 6-month lifetime) → code 'refresh_token_invalid'; anything else is
     * retryable → code 'refresh_transient'.
     */
    async refresh(refreshToken) {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          // Non-JSON error body — classify on status alone below. Machine code only.
          if (logger) logger.debug('oauth_refresh_error_body_unparseable', 'oauth', { status: res.status, reason: err?.name ?? 'parse_error' });
        }
    ...
    /**
     * Best-effort revocation — NEVER throws (disconnect must always be able to
     * clear local state). The caller logs a failure using the returned error
    ...
    async revoke(token, tokenTypeHint) {
      try {
        ...
        // Best-effort by contract: the error is RETURNED for the caller to log
        // (privacy: no token material). A breadcrumb here keeps the catch non-silent.
        if (logger) logger.debug('oauth_revoke_network_error', 'oauth', { reason: err?.name ?? 'network_error' });
