...
6. **הפעלה ואישור — מתג אחד** (round326) — במסך הגדרות העמודה יש מתג
   **"שמירה אוטומטית על העמודה"**. הדלקתו מדליקה auto-revert, ואם החשבון עדיין לא
   אישר OAuth — פותחת מיד את אישור הבעלים החד-פעמי בלשונית חדשה (אין צורך לפתוח
   `<BASE_URL>/oauth/start?st=<sessionToken>` ידנית). המצב מוצג חי: "מחובר ✓" מול
   "דרוש אישור", עם קישור **"חיבור מחדש"** למקרה של החלפת בעלים ראשי. מרגע האישור
   הביטולים נרשמים על שם המאשר כשהוא הבעלים הראשי. **צעדים 1–5 כאן הם החד-פעמיים
   ברמת האפליקציה (אתה, פעם אחת); צעד זה הוא מה שכל בעל עמודה עושה, בתוך האפליקציה.**
7. **רישום עמודות** — השמירה במסך ההגדרות רושמת את העמודה אוטומטית (idempotent),
   ומדליקת המתג נשמרת יחד איתה. לעמודות קיימות: להיכנס להגדרות, לוודא בעלים, ולשמור.
