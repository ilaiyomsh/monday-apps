/**
 * The settings shell shows the button to the column's OWNERS (round322) and to
 * nobody else.
 *
 ...
 *
 * The gate itself is pinned in services/settingsAccess.test.js; here it is mocked so
...
const mockLoadSettingsAccess = vi.fn();
...
vi.mock('../../services/settingsAccess', () => ({
  loadSettingsAccess: (...args) => mockLoadSettingsAccess(...args),
}));
...
vi.mock('../../services/mondayService', () => ({
  default: {
    getColumnConfig: (...args) => mockGetColumnConfig(...args),
...
describe('SettingsLauncher ownership gate', () => {
  beforeEach(() => {
    mockLoadSettingsAccess.mockReset();
    mockLoadIsBoardOwner.mockReset();
    mockGetColumnConfig.mockReset();
  ...
  it('renders the settings button for a column owner', async () => {
    mockLoadSettingsAccess.mockResolvedValue({ canConfigure: true, adopted: true });
  ...
  it('replaces the button with the owners-only statement for a non-owner', async () => {
    mockLoadSettingsAccess.mockResolvedValue({ canConfigure: false, adopted: true });
  ...
  it('states the owners-only message naming the COLUMN owners, in Hebrew', () => {
    expect(NON_OWNER_MESSAGE).toBe('רק בעלי העמודה יכולים לנהל את ההגדרות');
  });

  it('shows a failed check as an error, not as a permission denial, and still withholds the button', async () => {
    mockLoadSettingsAccess.mockRejectedValue(new Error('Network Error'));
  ...
  it('shows neither the button nor a message while the check is in flight', async () => {
    let resolveGate;
    mockLoadSettingsAccess.mockReturnValue(new Promise((resolve) => { resolveGate = resolve; }));
    ...
    resolveGate({ canConfigure: true, adopted: true });
    expect(await screen.findByRole('button', { name: 'הגדרות' })).toBeInTheDocument();
  });

  it('asks the gate about the board, column and user from the monday context', async () => {
    mockLoadSettingsAccess.mockResolvedValue({ canConfigure: true, adopted: true });

    render(<SettingsLauncher context={CONTEXT} />);

    await waitFor(() => {
      expect(mockLoadSettingsAccess).toHaveBeenCalledWith(
        { boardId: '5501', columnId: 'status', userId: '4002' },
        expect.objectContaining({ loadIsBoardOwner: expect.any(Function) }),
      );
    });
  });
