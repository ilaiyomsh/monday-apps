...
export default defineConfig({
  resolve: {
    // pnpm dev:mock sets VITE_MONDAY_MOCK=1 ג†’ the whole app transparently
    // uses the stub. Unset (tunnel/prod builds) ג†’ the real SDK.
...
context ג€” `column_view_settings` deliberately has **no `itemId`**, like
production.
...
test('shows an error toast on GraphQL failure', async () => {
  harness.failures.apiErrorNext = true; // next api() RESOLVES with { errors } ג€” like live
  // ...
});
```
...
- `harness.setTheme('light'|'dark'|'night'|'black')` ג€” emits a context event.
- `harness.setUser('admin'|'member'|'viewer'|'guest')` ג€” role flag variations.
- `harness.setContext(patch)` / `harness.setSettings(patch)` ג€” emit to listeners.
- `harness.seedStorage(key, value)` ג€” keys are scope-prefixed:
  `instance:<key>` / `global:<key>`.
- `harness.failures.*` ג€” see the toggle list at the top of `monday-sdk-stub.js`.
- `harness.calls` ג€” every `monday.execute(...)` recorded for assertions.
- `harness.apiHandlers.push({ match: 'my_mutation', data: {...} })` ג€” add
...
1. **Storage false-empty first read** (`storageFalseEmptyFirstRead`) ג€” the real
...
2. **GraphQL soft-error** (`apiErrorNext`) ג€” `monday.api` resolves (does not
   reject) with `{ errors }`. A missed check here is a silent write failure
   that looks like success.
3. **Hard/network failure** (`apiRejectNext`) ג€” the promise rejects.
4. **Theme variations** ג€” render in all four themes; monday users do use dark.
5. **Role variations** ג€” viewer/guest must not see owner-only affordances.
