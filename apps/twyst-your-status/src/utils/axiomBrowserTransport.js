...
/**
 * axiomBrowserTransport.js ג€” browser log transport for direct Axiom ingest.
 *
 ...
 * original shares it and fix both (error-guard SKILL.md ֲ§Self-correction, rule 4).
 *
 * Structural constraints (load-bearing):
 * - NO React imports, NO `import.meta` anywhere ג€” env is injected by the consumer
 ...
 *   for the injectable seams (`fetchFn`/`win`/`doc`) ג€” so the transport is fully
 *   testable in plain Node.
 * - `enqueue`/`flush` never throw to the caller. Every ship failure emits exactly
 *   one console.error starting '[axiom-transport] '.
 *
 * Event input contract: FLAT objects ג€” strings + finite numbers only (the sanitizer
...
/**
 * Sanitizer ג€” ONE precedence rule, applied per key of the flat input:
 ...
 * 3. remaining finite-number keys pass (key regex, max N numeric extras, NaN/ֲ±Infinity drop);
...
function sanitize(input, caps, allow) {
  ...
  for (const key of Object.keys(Object(input))) {
    if (TRANSPORT_OWNED.has(key)) continue;                 // transport-owned ג€” caller-supplied dropped
    ...
    // rule 4: objects, arrays, Errors, functions, symbols, booleans, null/undefined ג€” dropped
  }
  return out;
}

// HMR/StrictMode idempotency ג€” dispose-and-replace registry keyed by options.app.
const REG = new Map();
 ...
 * @param {string} [options.endpoint] - default api.axiom.co ג€” the proxy-pivot knob
...
export function createAxiomBrowserTransport(options) {
  ...
  // non-injected environment ג†’ inert handle. No registry interaction, zero listeners.
  ...
  /** Cut the oldest events from the queue into one batch (ג‰₪batchMaxEvents, ג‰₪batchMaxBytes). */
  ...
  /** Hidden path: single POST ג‰₪batchMaxBytes carrying the NEWEST events,
   ...
   *  re-queued. Returns success. Every failure ג†’ exactly one console.error breadcrumb. */
  ...
  async function doFlush(reason) {
    ...
    // Circuit breaker: open ג†’ no-op until the 60s window elapses, then half-open.
    ...
    if (breakerState === 'half-open') {
      ...
      try {
        ...
        if (ok) {
          breakerState = 'closed';
          consecutiveFailures = 0;
          // once per openג†’closed transition
    ...
    // closed ג€” the terminal hidden flush must NOT be starved by an in-flight routine
    // drain: cutBatchHidden() empties the queue synchronously, so running alongside
    // the drain is safe ג€” the drain loop re-checks queue.length after its await.
    if (reason === 'hidden') {
  ...
  /**
   * Enqueue pipeline (order is contractual): disposedג†’drop ֲ· sanitize + stamp `_time`
   * at enqueue ֲ· dedup (fixed window, transport-tag bypass, bounded map) ֲ· session cap
   * (exactly-once events_dropped meta, meta bypasses the cap) ֲ· queue cap drop-oldest ֲ·
   * schedule.
   */
  function enqueueEvent(input, meta) {
    ...
    if (!meta && shipped >= caps.sessionShipMax) {              // session cap ג€” meta bypasses it
      droppedSessionCap++;
    ...
    if (queue.length >= caps.batchMaxEvents) {                  // full batch ג†’ immediate flush
      clearTimer();
      flushNow('size');
  ...
  // Lifecycle listeners: visibilitychange(hidden) + pagehide ג†’ flush('hidden').
  const onVisibility = () => {
