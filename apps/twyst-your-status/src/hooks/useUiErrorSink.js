 ...
 *      `'ErrorBoundary:root'`) ג€” a render throw
 ...
 *
 * Toast mechanism is injectable ג€” this template imports NO toast library.
 ...
 * (search for `showToastRef.current(`) ג€” everything else in this file is
 ...
 * Not sticky by default ג€” adjust per app if errors need to persist longer.
 */
 ...
 * toasts can ever show ג€” after mount every new ERROR record still gets one.
 */
 ...
 * parser/catalog (e.g. a monday-API error-code-to-message map) ג€” that kind
 ...
 * to the raw technical `record.message` ג€” that is a log label, not a message
 * fit for an end user.
 *
 * @param {Object} record
 * @returns {string}
...
/**
 * UI Error Sink ג€” the single display path for logged errors.
 *
 * @param {Object} deps
 * @param {function(message: string, type: string, autoCloseMs: number, details: Object): void} deps.showToast
 *   Injected toast callback. Not imported from any specific toast library ג€”
...
export const useUiErrorSink = ({ showToast }) => {
    ...
    useEffect(() => {
        const uiHandler = (record) => {
            ...
            // boundary ג€” one display path per error, not two. The record is
        ...
        return unsubscribe;
        // Runs once on mount by design ג€” uiHandler reads showToast via a ref,
