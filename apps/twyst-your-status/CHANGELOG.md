...
## חיזוק שער האיכות — lint רואה מזהים תלושים + דיוק סורקי הניקוי (3.16.1)

**אפס שינוי התנהגותי.** קונפיגורציית ה-ESLint של ה-SPA קיבלה את מה שלשרת כבר היה:
`no-undef` + `no-unused-vars` + `react/jsx-uses-vars`/`jsx-no-undef` — מהיום import תלוש
או סימבול שהוזז נתפסים ב-lint במקום להישלח כ-`ReferenceError` ירוק (הפער שאילץ את פסילת
שני ממצאי המבנה בסבב הניקוי של 3.15.3). נוסף devDep אחד (`eslint-plugin-react`); ממצא
אמיתי אחד תוקן — import מת של `React` ב-`AppErrorBoundary.jsx`.

בנוסף, דיוק סורקים: תגי `@public` על exports של שכבת ה-boot של השגיאות (מסוננים במקור
מדוח knip — הם false positives קבועים שמוגנים ממילא ע"י שומר הניקוי) ו-`useUiErrorSink.js`
כ-entry. דוח knip של ה-SPA ירד מ-1 קובץ + 12 exports ל-0 קבצים + 4 — כולם מועמדים אמיתיים.

## round360 — שני תיקוני P0 לשומר + מימוש תוכנית ההשהיה (3.16.0)

מדידות חיות (2026-08-06, לוח הבדיקות של הבעלים) חשפו שני באגים חמורים מההשהיה עצמה,
ושניהם תוקנו יחד עם התוכנית המאושרת של סקירת הארכיטקטורה
(`docs/STATUS-GUARD-LATENCY-REVIEW.md` §4–§6).

**P0 ‏#1 — מקור הלייבלים: השומר חסם כל שינוי.** `GET_COLUMN_LABELS` בשרת שלף את
`settings_str`, שבו `labels` הוא **מפה**; `normalizeStatusLabels` מטפל רק בצורת
ה**מערך** של השדה המוקלד `settings` — זה שבוחר-הסטטוס בקליינט קורא. התוצאה: השרת
קיבל `labels = []`, כל מעבר סווג `not-offered`, ועם auto-revert דולק השומר החזיר
**כל** שינוי, חוקי או לא (נצפה חי: 3→1 ו-0→3 המותרים במפורש נחסמו; שוחזר
דטרמיניסטית עם הקלטים החיים). התיקון: השאילתה שולפת את `settings` המוקלד — אותו
שדה ואותו נורמלייזר כמו הקליינט, מקור אמת אחד (אומת חי מול API ‏2026-04). בנוסף,
fail-open אמיתי: לייבלים ריקים ⇒ שורת error ודילוג — בלי הערכה, בלי revert, בלי
רישום bypass — כך שעמודה לא-קריאה לעולם לא חוסמת שינויים לגיטימיים.

**P0 ‏#2 — מגן ה-echo: השומר נלחם בעצמו (סערת reverts של ~25 דקות).** ה-echo של
ה-revert הגיע אחרי שפג ה-TTL של 60 שניות (עיבוד של 40 שניות–4 דקות נצפה חי תחת
תור ה-lane) → ה-echo הוערך כשינוי אמיתי, נחסם, והשומר החזיר את ה-revert של עצמו —
פריטים התנדנדו 0↔3 כ-25 דקות עם הצפת התראות לבעלים ורישומי bypass. התיקון:
בדיקת ה-echo עברה **לראש** `process()`, לפני כל I/O (הסמן נושא עכשיו את `actorId`,
כך שאין צורך בקריאת rules); ה-TTL הועלה ל-10 דקות; הסמן **נספר** — שני reverts
לאותו ערך לפני ששני ה-echoes הגיעו ⇒ שניהם נבלעים; ו-revert שנכשל **מפרק** את
הסמן שחימש (echo לא יגיע — אחרת שינוי אמיתי הבא של הבעלים היה נבלע).

**עבודת ההשהיה** (בסיס שנמדד חי: 41.3 / 38.7 / 41.9 שניות ממוטציה ועד שהתא חוזר;
~4–5 שניות לכל קריאת רשת יוצאת, אחיד מול כל ה-backends):

- קריאות ה-GraphQL המותנות (labels ∥ teams ∥ item-context) רצות **במקביל**; תנאי
  ה-gating לא השתנו.
- **שחרור ה-lane אחרי ה-revert:** ההתראה ורישום ה-bypass רצים כזנב מנותק ומקבילי —
  ה-delivery הבא לאותו פריט לא ממתין להם. אירוע חסום שלא הוחזר ממשיך להמתין לרישום.
- **redelivery תחום:** ניסיון חוזר אחד בלבד (ברירת מחדל 5 שניות), רק על שגיאות
  storage חולפות; כל כשל אחר — ישר ללוג השגיאות.
- **שורת timing פר שלב:** `guard timing total=..ms tokens=..ms rules=..ms gql=..ms
  reread=..ms revert=..ms` לכל delivery מוערך — כולל נתיב ה-fail-open — ב-`code:logs`
  בלבד (החלטת בעלים: לא נשלח ל-Axiom).
- **cache לטוקני גישה** (`accountId:userId` → token+תפוגה, לעולם לא refreshToken)
  ‏+ **cache למצביע ה-reader** עם fallback למצביע המאוחסן כשהמצביע המוטמן מת —
  re-auth של בעלים אחר על אינסטנס אחר נקלט במקום עיוורון תמידי.
- **cache כללים ל-45 שניות — webhook בלבד:** מוטמן רק כשמועבר `accountId`
  מאומת-JWT; מסלולי sessionToken תמיד שולפים (מניעת הרעלת cache חוצת-דיירים).
- **מרוץ refresh חוצה-אינסטנסים:** ‏`invalid_grant` קורא שוב את הרשומה ומאמץ זוג
  שרוטט על ידי אינסטנס אחר, במקום לסמן reauth_required בטעות.
- **timeout פר ניסיון ל-GET של Vault:** ‏8000ms (הקריאה החיה נמדדה ~4–5 שניות);
  כתיבות/מחיקות בכוונה ללא timeout. חימום Vault ב-boot (fire-and-forget).

הערה: ~4–5 שניות פר קריאה היא תכונת **פלטפורמה** (שאלה פתוחה — §6.1: ‏connection
setup / egress); שורת ה-timing היא הכלי שיכריע אחרי הפריסה. הציפייה אחרי השחרור:
‏3→1 מותר (נחסם בטעות קודם), ‏3→0 מוחזר עם שורת timing, ‏echoes נבלעים בלי סערה.

**טסטים:** שלושה טסטי handler חדשים — שני echoes נספרים ונבלעים (והשלישי מוערך:
תקציב מדויק), revert שנזרק מפרק את סמן ה-echo שלו, ו-fail-open עדיין פולט את שורת
ה-timing. כולם אומתו אדומים דרך מוטציות ממוקדות: 4 מוטציות, 4 הריגות (כולל הסרת
ה-fallback של מצביע ה-reader מול `stores-caching.test.js`). סוויטת השרת 272/272,
שורש האפליקציה 885/885 (אחרי מיזוג develop עם ראונד הניקוי), ‏eslint נקי.
‏`server/package.json` נשאר 1.1.0 (אינו משוחרר עצמאית). בעקבות המיזוג עם פיצול
ה-barrel של הניקוי (`stores.js` → `stores/*`), שינויי ה-cache של round360 הועברו
לקבצים המפוצלים (`tokenStore.js`, ‏`rulesStore.js`) ואומתו מחדש עם 4 הריגות
מוטציה נוספות; ‏`REFRESH_CUSHION_MS` חזר להיות מיוצא מה-barrel (ה-API הציבורי
שקדם לפיצול, שסוויטת ה-caching נשענת עליו).

## ניקוי קוד מדורג — 7 batches, אפס שינוי התנהגות (3.15.3)

ריצה מלאה של workflow ניקוי חדש עם שער אנושי בין שלביו. **אפס שינוי מוצרי או
התנהגותי** — כל batch נדרש לעבור את מלוא סט ה-CI של הריפו מצומצם לאפליקציה
(error-wiring audit, eager-import audit, type-check, lint ×2, build ×2, הסוויטות
המלאות ×2, error-kit drift) לפני שנקבע כקומיט אחד בר-החזרה.

**מה השתנה בפועל:** `ColumnSettings.jsx` ירד מ-1,352 שורות ל-747 (−44.8%) ופוצל
ל-14 מודולים לפי תפרים קיימים — האפליקציה כבר לא מחזיקה אף קובץ מעל 1,000 שורות.
נמחקו 3 מודולים שאיש לא ייבא (`useQuery.js`, `DateRangeDisplay.jsx`,
`StatusChip.jsx`), ירדו exports מתים (20 → 12 לפי knip), הוסרו קשירות `React`
מיותרות מ-17 קבצי `.jsx`, ואוחדו כפילויות (שכפול 0.38% → 0.12%). `loadRoster`
עבר מ-`PersonPicker.jsx` ל-`src/services/rosterAccess.js` — שם כל קריאת monday
אחרת כבר יושבת — וקיבל 7 טסטים חדשים עם 3 מוטציות שנהרגו. סה"כ 1,048 → 1,075
טסטים, וה-bundle ללא שינוי ב-716 KB (קוד שאיש לא ייבא מעולם לא היה בו).

**מה נמצא בדרך ולא בוצע:** שני ממצאי מבנה נמחקו לפני ביצוע כי היו שוברים את
האפליקציה — אחד מהם היה זורק `ReferenceError` על המפסק שמזרים את השומר (חוזה
round326). 74% מממצאי הסקנר האוטומטי (20 מתוך 27) התבררו כ-false positives,
בעיקר שכבת השגיאות שהפלטפורמה קוראת לה ולא ה-imports.
