 ...
 * capped 200) + first stack frame + numeric context fields and short string ids.
 * Free-form payloads (request bodies, GraphQL responses, tokens) are NEVER shipped —
 * the field filter here mirrors the client transport's allowlist discipline.
 *
...
// Envelope enrichment — mirrors the client transport (axiomBrowserTransport.js): every
// shipped event carries `ver` (app version) and `sess` (per-process id) so records can be
// grouped by deploy and by process instance in the shared dataset.
const VER = process.env.npm_package_version || process.env.APP_VERSION || '0.0.0';
const SESS = Math.random().toString(36).slice(2, 10); // per process — generated once at module load
