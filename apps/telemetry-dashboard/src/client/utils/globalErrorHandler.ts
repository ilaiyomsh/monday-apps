 ...
 *
 * VENDORED from the canonical error-kit browser stack
 * (packages/error-kit/src/browser/globalErrorHandler.ts). The ONLY adjustment for this
 * standalone (no-workspace-deps) server app is the Logger type: error-kit imports it from
 * '../types'; here a minimal local interface (the two methods this module calls) replaces
 * that import so the file drops in without pulling the package in.
 *
...
/** The minimal logger surface this module calls — matches ./logger's (module, message, data) shape. */
interface Logger {
