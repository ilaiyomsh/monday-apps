...
// Columns: name | that cluster's date | radio column per action button.
...
const STYLES = `
      ...
      th.btn { color:#ffffff; font-size:13px; font-weight:normal; padding:6px 10px; min-width:88px; }
      td.pick { width:96px; padding:6px; }
      .pick-box { display:inline-block; cursor:pointer; line-height:0; }
      .pick-box input { width:18px; height:18px; margin:0; vertical-align:middle; }
      .go { margin:8px 0 4px; }
...
function renderClusterTable(section) {
  ...
  const btnHeaders = buttons
    .map(
      (b) =>
        `              <th class="btn" style="background:${escapeHtml(b.color)}">&#8207;${escapeHtml(b.label)}</th>`
    )
    .join('\n');
  ...
  const rows = section.tasks
    .map((task) => {
      ...
      const picks = buttons
        .map((b) => {
          return `              <td class="pick"><label class="pick-box"><input type="radio" name="${fieldName}" value="${escapeHtml(b.id)}"></label></td>`;
        })
        .join('\n');
      return `            <tr>
...
<html amp4email lang="he">
  ...
  <body dir="rtl">
    <div class="wrap">
      ...
      <p class="lead">&#8207;סמנו סטטוס חדש לכל משימה (עמודה אחת בלבד לשורה) ולחצו על אישור — כל העדכונים נשמרים מיד, בלי לצאת מהמייל.</p>
      <form method="post"
