...
5. **דוח ה-CSV (0.14.0):** בתיבת השולח עצמה (התיבה שחוברה במסך האדמין) לחפש
   מייל בנושא `deadline-confirm — דוח שליחה <slot>` עם צירוף
   `digest-summary-<slot>.csv`. **להוריד ולפתוח ב-Excel** — לא בתצוגה
   המקדימה של ג'ימייל: מה שנבדק כאן הוא ה-BOM, וג'ימייל מציג עברית נכון גם
   בלעדיו. לוודא: הכותרות בעברית (לא ג'יבריש), עמודה לכל מקבץ בסדר של מסך
   האדמין, ו**שורה לכל עובד בלוח העובדים** — כולל מי שלא נשלח לו, עם הסיבה
   בעמודה האחרונה.
6. **זמן הריצה (§7.3):** בתשובת `mapps scheduler:run -a 11704868 -n digest-send`
   (או בשורת הלוג `tenant run finished`) לרשום את `durationMs` ואת
   `recipients`. זו המדידה שפריט §7.3 ב-`docs/scheduling.md` מחכה לה — שם גם
   טבלת ההחלטה מה לעשות עם כל טווח.
