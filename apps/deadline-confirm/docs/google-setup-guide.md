...
> **הנקודה שחסמה אותנו בפועל (29/07/2026).** בלי מפתח DKIM משלכם, גוגל **כן**
> חותמת — אבל בדומיין ברירת מחדל שלה, למשל
> `twyst-co-il.20251104.gappssmtp.com`. DKIM עובר, ההתאמה ל-`From` נכשלת,
> DMARC נכשל, **ו-Gmail מסירה את חלק ה-AMP מההודעה בזמן המסירה** — ההודעה
> מגיעה כרשימת טקסט בלי כפתורים, ושום לוג באפליקציה לא יסביר את זה.
>
> **הבדיקה בהצצה אחת:** לפתוח הודעה שהמערכת שלחה ולקרוא את השורה
> **"חתום בידי" / "Signed by"** בפאנל הפרטים. אם היא מציגה `…gappssmtp.com`
> — DKIM לא מוגדר. אם היא מציגה את הדומיין שלכם — תקין.
>
> **שתי מלכודות בבדיקה:** דואר בין שתי תיבות באותו דומיין נמסר פנימית ולכן
> נושא **אפס** כותרות אימות — חייבים לבדוק מול נמען בדומיין אחר. וגם אחרי
> ש-DNS תקין, כל נמען צריך להוסיף את כתובת השולח ל-Developer settings שלו עד
> להשלמת רישום השולח מול גוגל.
>
> ליווי מפורט של מנהל ה-Workspace, כולל כל מצבי הכשל:
> **`docs/dns-email-auth-runbook.md`**.
