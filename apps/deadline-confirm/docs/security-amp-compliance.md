...
- "Don't allow third parties to directly author/send emails" — רלוונטי לנו:
  ההודעה נשלחת מחשבון Google ייעודי **של הלקוח** בדומיין של הלקוח, ולא משרת
  של Twyst. זה בצד הנכון של הכלל, אבל היועץ ישאל — כדאי להגיע עם התשובה.
  (עדכון 2026-08-04: ההרשאה בשלב הבדיקות היא `https://mail.google.com/` —
  SMTP XOAUTH2 אינו מקבל `gmail.send`; ראו `amp-email-verified-findings.md`
  §5 ואת ניסוח ההרשאות ב-`security-questionnaire-answers.md`. הטיעון כאן
  נשען על זהות-השולח-של-הלקוח, לא על צרות ה-scope.)
