...
- **Template-drift incident (2026-07-15, deadline-confirm):** the model above was right, but `templates/deploy-live.yml` was never updated to match it — discussions/team-people-column were hand-fixed after onboarding, the template kept the old bare `code:push --force -a <APP_ID>`. deadline-confirm onboarded from the stale template, so its first real live run pushed to the v3 DRAFT (16085102) and left live on v2 — caught only by post-deploy probing the live URL, not by the green run. Fix: the template now carries the resolve-LIVE_ID + pinned `-i` push, so every future onboard is correct by construction. Lesson: when the model changes, fix the TEMPLATE, not just the already-onboarded copies. The `awk -F'│' '$7 ~ /live/'` live-row parse depends on the CLI's box-drawing table output — re-verify it if `app-version:list` formatting ever changes.
- **Transient-retry (added to the live template 2026-07-15):** `code:push` intermittently fails with "Unexpected error occurred while communicating with the remote server" (the same transient `ship.sh` retries once); it failed a deadline-confirm live run that succeeded on re-run. The live template now wraps the push in a 2-attempt loop so a single transient no longer needs a manual `gh run rerun`.
...
- **BUMP ONCE PER CANDIDATE — NO bump-per-PR (owner rule 2026-07-14, after an agent needlessly burned 2.1.2→2.1.3 on draft iterations):** numbers count RELEASES. Bump only when the app's version equals main's (first change after a release) OR to raise magnitude (minor/major) when a bigger change joins — never another patch on a pending candidate. Draft iterations keep the number; the displayed SHA tells builds apart. `bump.sh` physically refuses default/patch bumps on a pending candidate; the corridor guard warns on raises.
