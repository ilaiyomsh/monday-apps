...
EG_UNION_SELECTOR="CatchClause > BlockStatement:not(:has(CallExpression[callee.object.name='logger'])):not(:has(ThrowStatement)):not(:has(CallExpression[callee.name='showErrorWithDetails'])):not(:has(CallExpression[callee.name='next'])):not(:has(CallExpression[callee.name='logError']))"
...
# Resolution runs with the candidate dir as Node's WORKING DIRECTORY and the module
# name passed via argv — never interpolated into the JS source. On Windows Git Bash
# an MSYS `/c/...` path embedded in a `paths:[...]` literal does not resolve, which
# silently reported eslint-plugin-promise as missing (known-issues 2026-07-27).
eg_resolve_module() {
  local mod="$1" fallback="${2:-}" base out
  for base in "$APPS_ROOT" "$fallback"; do
    [ -n "$base" ] && [ -d "$base" ] || continue
    out="$( (cd "$base" && node -e 'try{process.stdout.write(require.resolve(process.argv[1]))}catch(e){}' "$mod") 2>/dev/null || true )"
...
eg_gen_config() {
  ...
  local rules_json
  # Node, not jq: the audit path must stay self-contained on Windows Git Bash,
  # where jq is frequently absent (known-issues 2026-07-27).
  rules_json="$(node - "$TEMPLATE" "$EG_UNION_SELECTOR" "$hp" <<'NODE'
const fs = require('fs');
const [templatePath, selector, hasPromiseRaw] = process.argv.slice(2);
const template = JSON.parse(fs.readFileSync(templatePath, 'utf8'));
const rules = { ...template.rules };
if (hasPromiseRaw !== 'true') delete rules['promise/catch-or-return'];
rules['no-restricted-syntax'] = rules['no-restricted-syntax'].map((entry, i) =>
  i === 1 && entry && typeof entry === 'object' ? { ...entry, selector } : entry
);
