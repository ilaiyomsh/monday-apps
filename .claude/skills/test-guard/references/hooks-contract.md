...
3. Dedupe `T` (sort -u). **A touched path that no longer exists on disk is skipped** before any gate lookup (amendment 13) — `touched.txt` is append-only, so a module created and then deleted within the same session has no code left to cover and no reachable exit. Count the skips. For every path that DOES exist, compute covered/uncovered exactly per §4.2 step 4 (same lookup, same verdict rule). Collect uncovered module list `U`.
4. `U` empty → exit 0, silently **unless paths were skipped in step 3**, in which case emit the informational `additionalContext` line naming the count and the paths (the stop still proceeds — no `decision`). When `U` is non-empty, the same line is appended to the block reason / yield warning below.
...
- Hooks: pipe hand-crafted stdin JSON with `REDGREEN_STATE_ROOT` pointed at a fixture dir. For `stop-gate.sh` this protocol is now a committed, re-runnable script — `bash .claude/skills/test-guard/hooks/tests/verify-stop-gate.sh` (19 assertions, throwaway state root per case, bash 3.2-safe, no deps beyond bash + coreutils; jq only for the fallback-JSON assertions, which skip without it). **It runs in CI** as the `test-guard hook fixtures` job (`ci.yml`, checkout-only, no pnpm install), so a regression in the enforcement layer cannot land silently — still run it locally after any stop-gate or `cmd_waive` change and paste the summary. Assert (a) deny/block JSON emitted for a locked/uncovered fixture, (b) silent exit 0 for clean fixtures, (c) silent exit 0 for garbage stdin (`echo 'not json' | hook` must exit 0, empty stdout), (d) `echo '{}' | hook` exits 0 silently, (e) stop-gate honors the counter across repeated invocations and yields with the warning at `MAX`, (f) amend-intent lets exactly one edit through (second identical PreToolUse input is denied again).
...
12. **`survivors.sh resolve` no longer no-ops silently when no `kills.log` line matches** (2026-08-02, closes the usability trap flagged at the end of amendment 11, hit the same day on discussions round319). The trap: `spotcheck-fire` logs `SURVIVED|<mutation description>`, `record` defaults `--desc` to the `--hypothesis` (a different sentence by nature — one describes the injected bug, the other the missing assertion), so `resolve` finds nothing to rewrite, prints *"fine if this record was never logged with --log-kills"*, and exits 0. The record reads KILLED, the `^SURVIVED|` line lives on, and the verdict stays `NOT DONE` for a reason the message actively reassures you about.

    The candidate fix in amendment 11 (default `--desc` to the newest unresolved line) was **rejected as unsafe**: with two open survivors it would close whichever happens to be newest, which is exactly the silent mis-resolution the idempotency guard above already exists to prevent. Implemented instead, on both ends and closing nothing on its own:
    - **`resolve`**: when the fixup matches nothing AND `kills.log` still holds unresolved `SURVIVED|` lines, it prints them, prints the exact re-record command, and **exits 1**. A resolve that resolved nothing is not a success. When there are no unresolved lines it stays a quiet exit-0 no-op (the genuinely-never-logged case), with the message reworded so it no longer reads as an all-clear for the other one.
    - **`record`**: when `--desc` is omitted and `kills.log` has unresolved lines, it names them and says they will not be closed by this record — while the agent still has the fire's wording on screen.

    Verified in a sandboxed fixture (`REDGREEN_STATE_ROOT`, three cases): (A) record-without-`--desc` then resolve → both warnings fire and resolve exits 1; (B) record with the exact `--desc` → line rewritten to `SURVIVED-RESOLVED`, exit 0 (true positive intact); (C) a record that was never logged, no unresolved lines → quiet exit 0. The fixture also caught a defect in the first cut of the fix itself: `$(grep -c … || echo 0)` yields the two-line string `"0\n0"` when there is no match (grep prints 0 *and* exits 1), which made the arithmetic test die with a syntax error — replaced with `grep -q`.

13. **A touched path that no longer exists on disk is no longer gated** (2026-08-04, hit for real on branch `claude/code-scan-security-wx0cgv`). `route-nudge` appends every product-source touch to `touched.txt` **append-only** (§4.2 step 3), and `stop-gate`'s uncovered-collection loop resolved a gate dir per touched path without ever asking whether the path still existed. So a module created and then **deleted within the same session** — here `security/xss-fuzz.mjs`, a scratch fuzz script superseded by a proper in-suite vitest test — kept blocking Stop with **no sanctioned exit at all**: `green` has nothing to run, `spotcheck-arm` has nothing to mutate, and `waive` died with `GATE ERROR: test file not found` because `resolve_env` stats the file. The only escapes left were the loop-protection yield ("recorded, not forgiven") and hand-editing `touched.txt` — neither a recorded, auditable outcome.

    Fixed on both ends:
    - **`stop-gate.sh`**: the collection loop skips any touched path failing `[ ! -e ]`, *before* the gate lookup — a deleted module has no code left to cover, so there is nothing to gate. Coverage for every path that still exists is byte-for-byte unchanged (same lookup, same §2.4 verdict, same block). The skip is **reported, never silent**: `additionalContext` names the count and the paths, either standalone (when nothing else blocks — the stop still proceeds, no `decision` key) or appended to the block reason / yield warning. Reporting the skip is the point: it is a decision the gate made on the agent's behalf, so it belongs in the session record.
    - **`redgreen.sh waive`**: a path that does not exist now resolves **state-only** (like `verdict`/`amend-intent`) instead of dying, so a waiver for a deleted module is recordable as belt-and-braces audit trail. Nothing is weakened — it records a reason exactly as the existing-file path does, and a waiver on a path no product code lives at covers no product code; `status-all` already annotates such a `test.path` as `(missing)`. The existing-file branch still runs the full `resolve_env` (runner detection + preflight), verified unchanged: `waive` on a real file in a runner-less project still exits 2 with the vitest/jest error.

    Verified with a committed fixture verifier, `hooks/tests/verify-stop-gate.sh` (§5): 19 assertions, fresh throwaway `REDGREEN_STATE_ROOT` per case. **8 fail against the pre-fix scripts and pass after** (deleted-path blocks / skip unreported / deleted module listed as uncovered / yield warning missing the note / all three waive assertions, reproducing the exact `GATE ERROR: test file not found`), and **8 pass on both sides** — the no-weakening half: an existing untracked module still blocks, a mixed session still blocks and still names the surviving module, a DONE module still allows the stop with empty stdout, garbage stdin and `{}` still exit 0 silently, and the loop-guard still yields at MAX. shellcheck-clean at `--severity=warning`.

    **The verifier runs in CI** (owner request, same PR): job `test-guard hook fixtures` in `ci.yml` — checkout-only, no pnpm install, about a second. The stop gate is the layer that physically holds a session open, so a regression in it removes enforcement silently; a green pipeline with no gate behind it is precisely what a diff review cannot catch. Placed as its own job rather than a step in `type-check, lint, build` so dependency resolution can neither delay nor break it.

    Three assertions were added for CI's sake, covering the hook's **no-jq fallback** — most machines have jq, so the hand-rolled `printf` + `json_escape` path is the one that rots unnoticed, and `emit_context` added a new caller to it. Case H runs the hook with `PATH` reduced to a bin dir of symlinks (no jq) against a touched path containing both a double quote and a backslash, then parses the output with the real jq and round-trips the path through `.reason`. It kills two mutations: dropping `json_escape`'s backslash clause (JSON still parses — `\b` is a valid escape — but the path comes back mangled, so only the round-trip assertion catches it), and inverting the deleted-path condition to `[ -e "$s" ]` (8 assertions fail across A/B/C/D/H). Building the case also caught a defect in the harness itself: reducing `PATH` hid `bash` too, so the hook exited 127 and the case failed for the wrong reason — it now invokes an absolute interpreter path.
