...
- Hooks: pipe hand-crafted stdin JSON with `REDGREEN_STATE_ROOT` pointed at a fixture dir. For `stop-gate.sh` this protocol is now a committed, re-runnable script — `bash .claude/skills/test-guard/hooks/tests/verify-stop-gate.sh` (16 assertions, throwaway state root per case, bash 3.2-safe, no deps). It is deliberately NOT wired into `ci.yml`; run it after any stop-gate or `cmd_waive` change and paste the summary. Assert (a) deny/block JSON emitted for a locked/uncovered fixture, (b) silent exit 0 for clean fixtures, (c) silent exit 0 for garbage stdin (`echo 'not json' | hook` must exit 0, empty stdout), (d) `echo '{}' | hook` exits 0 silently, (e) stop-gate honors the counter across repeated invocations and yields with the warning at `MAX`, (f) amend-intent lets exactly one edit through (second identical PreToolUse input is denied again).
