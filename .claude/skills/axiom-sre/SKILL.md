---
name: axiom-sre
description: Expert SRE investigator for incidents and debugging across any app wired into Axiom/Grafana/Pyroscope/Sentry. Uses hypothesis-driven methodology and systematic triage. Use for incident response, root cause analysis, production debugging, or log investigation — and for the user's phrases "תקלה בפרוד", "לוגים", "למה זה נפל", "Axiom".
---
...
## 0. Multi-Project Config (this skill is shared across apps)

This skill is a shared, project-agnostic toolkit (promoted here from `Axis/sync-calender`) — it never hardcodes dataset names. It discovers datasets/datasources/applications per environment at runtime (Golden Rule #9), reading connection config from `~/.config/axiom-sre/config.toml` (global, per-user, managed by `scripts/init`).

**Before investigating a specific app**, figure out which Axiom env alias / dataset naming convention that app uses:
1. Check the app's `.env` for `AXIOM_URL`, `AXIOM_API_KEY` / `AXIOM_READ_TOKEN`, `AXIOM_DASHBOARD_KEY` (these tell you *which* Axiom org/instance the app ships to — never print their values, see Golden Rule #8).
2. Check the app's `CLAUDE.md` for a documented Axiom env alias or dataset naming note.
3. If neither is documented, fall back to the app-slug convention used by `add-to-status-hub` (`<slug>-prod`, e.g. dataset `sync-calender-prod` for the `sync-calender` app) and confirm with `scripts/discover-axiom` — never guess.

**Documented default for `sync-calender`** (the app this skill was promoted from): env alias `prod` in `config.toml`, Axiom instance identified by that app's `AXIOM_URL`/`AXIOM_API_KEY`. No dataset name is hardcoded even for this app — always resolve it via `scripts/discover-axiom prod` per Golden Rule #9. Treat this paragraph as the fallback documented default, not a shortcut around discovery.
...
## Triage Crib — known monday.com error signatures

Check these BEFORE a full investigation loop — they're confirmed, recurring root causes for monday-integration apps (fast-path, still verify with a query per Golden Rule #1):

| Symptom in logs | Root cause | Fix |
|---|---|---|
| `UNAUTHORIZED_FIELD_OR_TYPE`, `status=200`, `op=Notify` (or any mutation on a newly-scoped field) | monday returns GraphQL auth errors with HTTP 200 — the stored OAuth token predates the scope the mutation needs (e.g. `notifications:write` added after the token was granted) | Not a code bug. Have the affected account reconnect/re-authorize the app so a fresh token is issued with the current scopes. |
| `FIELD_MINUTE_RATE_LIMIT_EXCEEDED` | A mirror or formula column's `display_value` fans out into a per-cell field read that multiplies complexity per item fetched | See `monday-api` skill's `references/complexity.md` for the mirror/formula complexity model and how to batch/limit fields to stay under budget |

Always confirm the exact `extensions.code` / `status` from the query result before concluding — this table is a lead, not a verdict (Golden Rule #3).

## See also

- **`add-to-status-hub`** — writes the dual-transport Axiom logger into an app and registers it in the status-hub dashboard. Use that skill to get an app's logs *into* Axiom in the first place (tag/message/level/acc/usr/obj/cfg/prv conventions); use *this* skill to query and triage what lands there. The two form one instrumentation → investigation loop.

## Self-improvement — when the skill itself fails you

When this skill's tooling misfires — a `scripts/discover-*` or query script erroring against the live Axiom/Grafana/Pyroscope/Sentry setup, a `reference/*.md` recipe or query pattern that no longer matches the current schema, or a hypothesis playbook (Triage Crib, cognitive traps) that leads you astray:

1. Treat the failure as a finding about the SKILL, not just the incident — don't silently hand-write a one-off workaround query and move on.
2. Same-session recording is mandatory (project standing rule): fix the script or `reference/` page in the session it's discovered, recording what broke and the verified fix — same discipline as a `kb/patterns.md` entry, but about the skill itself rather than the investigated app.
3. If a real fix is out of scope right now, record a known-gap note (with fix direction) and surface it to the user before closing out.
4. Never bypass a Golden Rule (e.g. discovery-before-query, Rule #9) just to get unblocked — narrow or skip a check only with proof it was safe.
