...
| `code:status -i <VERSION_ID>` prints ONLY the per-deploy **Version URL** (changes on every `code:push` — verified: sync-calender status shows `b4f58-…` while its stable URL is `live1-…`). The static **Live URL** is minted on the FIRST LIVE release, serves the live version only, and is visible ONLY in Developer Center → Host on monday → Server-side code → General (no CLI/API field, docs-confirmed). AUTH CONFIG RULE: OAuth redirect + BASE_URL must use the static Live URL in production; during draft-only testing the Version URL is the only option — re-set BASE_URL + redirect after every draft push, and register redirect URLs additively (the dev center allows multiple). | docs+live-verified 2026-07-15 |
