...
| monday code exposes TWO urls per version: the per-deploy **Version URL** (`e47e2-service-…`) and the static **Live URL** (`live1-service-…`). `code:status -i <VERSION_ID>` shows the Version URL always, and grows a SECOND url column with the Live URL once it exists — on deadline-confirm the Live URL column was ABSENT after the first successful deploy and PRESENT from the second one, and it serves the draft deployment too (health+admin verified). AUTH CONFIG RULE: point OAuth redirect + BASE_URL + feature customUrl at the static `live1-…` URL as soon as it appears — it never changes; the Version URL may. (Docs claim the Live URL is minted only on first live release and is dev-center-only — both claims contradicted by live observation.) | live-verified 2026-07-15, app 11704868 |
