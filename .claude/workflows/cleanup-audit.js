export const meta = {
  name: 'cleanup-audit',
  description: 'Stage 1 of the cleanup workflow for a registered app (default twyst-your-status; {"app":"discussions"} selects the other): scan, adversarially verify, judge, and write CLEANUP_PLAN.md. Read-only for source.',
  whenToUse: 'After scripts/cleanup/baseline.sh has passed for that app (CLEANUP_APP=<app> bash scripts/cleanup/baseline.sh). Produces <app>/.cleanup/CLEANUP_PLAN.md with every batch pending — a human sets batches to approved. Args: optional {app} (registered app key) and {target} subdirectory INSIDE the app (first run: pilot one subdirectory), or a bare path string.',
  phases: [
...
// ---------------------------------------------------------------------------
// SCOPE. This workflow serves exactly ONE registered app per run, selected by args.app
// (default twyst-your-status). The registry below must match scripts/cleanup/cleanup-env.sh
// — the guards dispatch on the same key (CLEANUP_APP), so an app not registered in BOTH
// places cannot run. Every path below is inside the selected app, and the target is
// validated against it before a single agent starts — a cleanup that quietly widens to a
// sibling app is the failure mode this whole package is built to prevent.
const APPS = {
...
if (!APPS[appKey]) {
  return { error: `Unknown app "${appKey}" — registered cleanup apps: ${Object.keys(APPS).join(', ')}. Register a new one in scripts/cleanup/cleanup-env.sh + scripts/cleanup/env/<app>.sh first.` }
}
...
if (!target.startsWith(APP)) {
  return { error: `Refusing to run: target "${target}" is outside ${APP}. This run is scoped to ${appKey} only.` }
}
...
const scan = await agent(
  `Run the deterministic cleanup scanners for ${appKey}. Target for the file-level scans: ${target}. Output suffix: none (this is an audit run, not a verify re-scan). Save everything under ${RAW}/.\n\n` +
...
if (!scan.knipTrustworthy) {
  return {
    ...
    fix: `Check the knip.jsonc file(s) under ${APP} entry points, run "pnpm dlx knip@5.88.1 --directory ${APP}" by hand, then re-run this workflow.`,
...
const verifyChain = async () => {
  ...
  const parsed = await agent(
    `Parse the knip JSON reports into individual, id'd findings for verification. Read ${RAW}/knip-spa.json (workspace "spa") and — only if it exists (a client-only app has no server workspace and no such report) — ${RAW}/knip-srv.json (workspace "server").\n\n` +
...
const judgeArea = area => async () => {
  return agent(
    `Audit the "${area}" focus area of ${appKey}. Target directory: ${target}.\n\n` +
...
const plan = await agent(
  `Write the cleanup plan for ${appKey} to ${STATE}/CLEANUP_PLAN.md.\n\n` +
  `INPUTS\n` +
